...
public class RuleVersion {
    ...
    public void submitApproval() {
        if (status != RuleStatus.DRAFT && status != RuleStatus.TESTING && status != RuleStatus.REJECTED) {
            throw new IllegalStateException("only draft, testing, or rejected versions can be submitted for approval");
        }
        status = RuleStatus.PENDING_APPROVAL;
    }

    public void markApproved(String reviewedBy) {
        if (status != RuleStatus.PENDING_APPROVAL) {
            throw new IllegalStateException("only pending approval versions can be approved");
        }
    ...
    public void markRejected() {
        if (status != RuleStatus.PENDING_APPROVAL) {
            throw new IllegalStateException("only pending approval versions can be rejected");
        }
    ...
    public void publish(String approvedBy, boolean gray) {
        if (status != RuleStatus.APPROVED) {
            throw new IllegalStateException("rule version must be approved before publishing");
        }
