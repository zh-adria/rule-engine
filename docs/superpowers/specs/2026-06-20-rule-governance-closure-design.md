...
Use `RuleStatus.ARCHIVED` for archived rule versions where the implementation already models version status. For rule definition archive state, add a persisted field on `rule_definition`, such as `archived BOOLEAN NOT NULL DEFAULT FALSE`, to avoid overloading version status.
